...
import net.corda.samples.corda.did.DidDocumentFailure.InvalidDidFailure
import net.corda.samples.corda.did.DidDocumentFailure.InvalidDocumentJsonFailure
import net.corda.samples.corda.did.DidDocumentFailure.InvalidTimeStampFormatFailure
	...
	fun publicKeys(): DidDocumentResult<Set<QualifiedPublicKey>> = json.getMandatoryArray("publicKey").map { keys ->
		keys.filterIsInstance(JsonObject::class.java).map { key ->
			...
			val listOfEncodings = arrayOf(
				...
				PublicKeyEncoding.PublicKeyBase64,
				PublicKeyEncoding.PublicKeyHex,
				PublicKeyEncoding.PublicKeyMultibase,
				PublicKeyEncoding.PublicKeyPem,
				PublicKeyEncoding.PublicKeyJwk
			)
